              ...
              <div>
                ...
                {shortestPathsSorted.map((path, index) => {
                  const locationDetails = locations.find(
                    (loc) =>
                      loc.locat_id.toString() ===
                      path.properties.end_name.toString()
                  )
                  return (
                    <button
                      key={index}
                      className="path-card"
                      onClick={() =>
                        handlePathCardClick(path.properties.end_name)
                      }
                    >
                      ...
                      {locationDetails ? (
                        <div className="location-details">
                          <div className="location-name font-medium text-base">
                            {locationDetails.locat_name}
                          </div>
                          <div className="location-address text-sm text-gray-600 mb-2">
                            {locationDetails.address}
                          </div>

                          {/* 顯示展覽或課程 */}
                          {selectedType === 'exhibition' &&
                            locationDetails.exhibitions && (
                              <div className="location-exhibitions">
                                <div className="font-medium mb-1">
                                  Exhibitions:
                                </div>
                                <div className="space-y-3">
                                  {locationDetails.exhibitions.map(
                                    (exhibition, index) => (
                                      <div
                                        key={`exhibition-${
                                          exhibition.e_id || index
                                        }`}
                                        className="exhibition-item border-l-2 border-blue-500 pl-2"
                                      >
                                        <div className="exhibition-name font-medium">
                                          {exhibition.e_name}
                                        </div>
                                        <div className="exhibition-dates text-xs text-gray-600">
                                          {formatDate(exhibition.e_startdate)} -{' '}
                                          {formatDate(exhibition.e_enddate)}
                                        </div>
                                      </div>
                                    )
                                  )}
                          ...
                          {selectedType === 'courses' &&
                            locationDetails.courses && (
                              <div className="location-courses">
                                <span>Courses:</span>
                                <ul>
                                  {locationDetails.courses.map(
                                    (course, index) => (
