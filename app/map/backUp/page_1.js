...
export default function Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        // const [routesRes, stationsRes, districtsRes, pathsRes, lineStationsRes] = await Promise.all([
        ...
        const lineStationsData = await lineStationsRes.json();
        ...
        const lineData = lineStationsData.mrt_lines.find(line => line.line === selectedMRT);
        setSelectedLineStations(lineData ? lineData.stations : []);
  ...
  const handleLineSelect = useCallback((line) => {
    ...
    setSelectedStation("");
    setSelectedDistrict("");
    // Trigger loading new line stations
  }, []);
